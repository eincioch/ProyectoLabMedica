...
namespace Integration.DAService.DA_CtasCtesMedica
{
    public class DA_CuentaCorriente
    {
        ...
        //----------------------------------------------------------
        // Update CuentaCorriente y AdmSolAtencion (Anulando Recibo)
        //----------------------------------------------------------
        public bool Upd_CuentaCorriente_And_AdmSolAtencion(BE_ReqCuentaCorriente Request)
        {
            bool exito = false;
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(Cadena))
                {
                    ...
                    using (SqlCommand cm = new SqlCommand())
                    {
                        cm.CommandText = "[usp_Upd_CuentaCorriente_And_AdmSolAtencion]";
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.Parameters.AddWithValue("cCtaCteRecibo", Request.cCtaCteRecibo);
                        ...
                        else throw new ApplicationException("Se ha producido error en procedimiento almacenado: [usp_Upd_CuentaCorriente_And_AdmSolAtencion]; Consulte al administrador del sistema");
                    }
                }
