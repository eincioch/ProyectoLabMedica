...
Public Class FormMaTipoServicios
    ...
    Private Sub FormMaTipoServicios_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        'Update 10-Oct-2015 - EINCIO
        'LlenaDataCombo(CboTipoProceso, ObjConst.Get_Constante("C", CLng(TokenByKey(CboTipoProceso.Tag, "nConCodigo"))), "nConValor", "cConDescripcion")
        LlenaDataCombo(CboEstado, ObjConst.Get_Constante("C", CLng(TokenByKey(CboEstado.Tag, "nConCodigo"))), "nConValor", "cConDescripcion")
    ...
    Private Sub TreeView_AfterSelect(sender As System.Object, e As System.Windows.Forms.TreeViewEventArgs) Handles TreeViewC.AfterSelect
        ...
        If dtGenerico.Rows.Count > 0 Then
            With dtGenerico
                ...
                CboTipoProceso.SelectedIndex = IIf(CInt(.Rows(0)("nProceso")) = 0, 0, 1)
                TxtNHorasProceso.Text = .Rows(0)("nHorasProceso")
                CboEstado.SelectedValue = .Rows(0)("nEstado")
    ...
    Private Sub c1CmdGrabar_Click(sender As Object, e As C1.Win.C1Command.ClickEventArgs) Handles c1CmdGrabar.Click
        ...
        Try
            ...
            If ValidaCamposObligatorios() Then
                ...
                Request.nSeuCodigo = Convert.ToInt32(TxtCodCPT.Text)  '0 REM CLng(CboModAsiento.SelectedValue)
                Request.nCtaCteSerFacturable = 0 REM CLng(CboFacturable.SelectedValue)
                Request.nCtaCteTipoCta = CLng(TxtnCtaCteTipoCta.Text)  'CboTipoCta.SelectedValue
                ...
                Request.nProceso = CInt(CboTipoProceso.SelectedIndex)
                Request.nEstado = CInt(CboEstado.SelectedValue)
                Request.nHorasProceso = CInt(TxtNHorasProceso.Text)
                ...
                If nFlag = 0 Then
                    ...
                    traza = "Nuevo Servicio: -Item: " & LblJerarquia.Text.ToString + " -Descripcion: " & LblDescr.Text.ToString
                    Trace_Transaccion(CInt(Trace.RegistroServicio), StrUser, traza)

                ElseIf nFlag = 1 Then
                    REM Update                    
                    ResultadoG = ObjCtaCteServ.Upd_CtaCteServicio(List)

                    traza = "Cambio de Precio:  -Nuevo Importe: " & TxtImpDef.Text.ToString & " -Item: " & LblJerarquia.Text.ToString + " -Descripcion: " & LblDescr.Text.ToString
                    Trace_Transaccion(CInt(Trace.ActualizandoServicio), StrUser, traza)
                End If

                '------------------------------------------------
                '-- Grabando Precios Costo segun Tipo de Atencion
                '------------------------------------------------
                'Codigo queda obsoleto por utilizar precio unico y aplicar porcentanjes por Lab o Convenio
                ...
                If ResultadoG Then 'Or Resultado > 0 Then
    ...
    Private Sub CboTMoneda_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles CboTMoneda.KeyPress ', CboTipoCta.KeyPress
