...
Public Class FormMaParentesco
    ...
    Function ValidaCamposObligatorios() As Boolean
        ...
        If FromParentesco = 1 Then 'for FormMaPersona
            If TokenByKey(StrPasaValores, "cPerCodigo") = "" Then ', CType(Me.Owner, FormPrcSolAtencion).LblcPerCodigo.Text, TokenByKey(StrPasaValores, "cPerCodigo")
                MsgInfoCampoObligatorio("Código Ref.", CboTipoPer)
                Exit Function
            End If
        Else 'for FormPrcSolAtencion
            If CType(Me.Owner, FormPrcSolAtencion).LblcPerCodigo.Text = "" Then 'TokenByKey(StrPasaValores, "cPerCodigo") = "" Then
                MsgInfoCampoObligatorio("Código Ref.", CboTipoPer)
                Exit Function
            End If
        End If
        ...
        If CboSexoTEmp.SelectedIndex = -1 Then
            ...
            '    Exit Function
        End If
        If Convert.ToInt32(LblnEdad.Text) < 18 And CboTipoPer.SelectedValue = nConTipoPersona.nPerNatural Then
    ...
    Private Sub FormMaParentesco_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        If nFlagNewPariente = CInt(AccionForm.Add) Then
        ...
        ElseIf nFlagNewPariente = CInt(AccionForm.Edit) Then
    ...
    Private Sub BtnAceptar_Click(sender As System.Object, e As System.EventArgs) Handles BtnAceptar.Click
        ...
        Try
            ...
            If nFlagNewPariente = CInt(AccionForm.Add) Then '1: Add New Pariente   
            ...
            If ValidaCamposObligatorios() Then
                ...
                Using scope As New TransactionScope
                    ...
                    Request.cPerCodigo = Trim(LblcPerCodigo.Text)
                    ...
                    'Parentesco
                    If FromParentesco = 1 Then 'for FormMaPersona
                        Req_PerParentesco.cPerCodigo = TokenByKey(StrPasaValores, "cPerCodigo")
                    Else 'for FormPrcAdmSolicitud
                        Req_PerParentesco.cPerCodigo = CType(Me.Owner, FormPrcSolAtencion).LblcPerCodigo.Text
                    End If

                    Req_PerParentesco.cPerParCodigo = Trim(LblcPerCodigo.Text)
                    ...
                    If MsgGrabar() = System.Windows.Forms.DialogResult.Yes Then
                        ...
                        If Resultado And ResultadoP Then
                            StrPasaValoresParentesco = vbNullString 'seteo la variable 
                            StrPasaValoresParentesco = "cPerCodigo=" & LblcPerCodigo.Text & ";"
                            StrPasaValoresParentesco = StrPasaValoresParentesco & "cPerIdeNumero=" & TxtNroDocu.Text & ";"
                            StrPasaValoresParentesco = StrPasaValoresParentesco & "cNombCompleto=" & TxtApePat.Text & " " & TxtApeMat.Text & ", " & Txt1Nombre.Text & ";" '& Txt2Nombre.Text & " " & Txt3Nombre.Text & ";"
                            StrPasaValoresParentesco = StrPasaValoresParentesco & "nPerJurTipInversion=" & CboSexoTEmp.SelectedValue & ";"
                            StrPasaValoresParentesco = StrPasaValoresParentesco & "nPerJurSecEconomico=1" ' & CboEstadoCivil.SelectedValue
