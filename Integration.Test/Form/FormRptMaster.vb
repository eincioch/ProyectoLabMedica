...
Public Class FormRptMaster
    ...
    Private Sub FormRptMaster_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Dim ldr As List(Of IDataReader)

        Dim Response As New List(Of BE.Interface.BE_Res_Interface)()

        Dim Request As New BE.Reportes.ReqRptParametros
        Dim Servicio As New BL.BL_Reportes.BLRptMaster

        Request.nIntClase = 1022
        Request.cIntJerarquia = "MAD"
        Request.cPerCodigo = StrUser

        'ldr = Servicio.Get_List_Report_by_cIntJerarquia_and_cPerCodigo(Request)

        If CType(ldr(0), SqlClient.SqlDataReader).HasRows = True Then
            ldr(0).Read()

            FillTv(TreeView1, ldr(0), "75.CFG", Nothing)
            ldr(0).Close()
        End If
        ldr = Nothing
