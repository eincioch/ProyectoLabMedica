...
Module mdl_Functiones
    ...
    Function ValidarApertura(v_nOpeCodigo As Integer) As Boolean

        Try
            Dim estado As Boolean = False

            'Verifica si tiene fecha de apertura
            ...
            perOperacion.nOpeCodigo = v_nOpeCodigo '1--apertura,2--Cierre

            Dim dataTable As New DataTable

            dataTable = gestionarPerOperacionServicio.Get_PerOperacion(perOperacion)
    ...
    Public Function ObtenerParametroConfiguracion(ByVal cIntJerarquia As String) As Integer

        ObjetoSistema.cPerCodigo = StrcPerJuridica
        ObjetoSistema.nSisGruCodigo = nSisGruCodigo.AdmisionMedica
        ObjetoSistema.cIntJerarquia = cIntJerarquia '"28.CFGGENPORCMAX"

        cDT = ObjSistemaServicio.Get_General_Get_Configuracion_By_PerJur_Modulo_Jerarquia(ObjetoSistema)

        If cDT.Rows.Count > 0 Then
            ObtenerParametroConfiguracion = Convert.ToInt32(cDT.Rows(0)("cIntNombre"))
        Else
            ObtenerParametroConfiguracion = 0
        End If

        Return ObtenerParametroConfiguracion

    End Function

    Public Function Trace_Transaccion(ByVal nOpeCodigo As Integer, ByVal cPerCodigo As String, ByVal cTraDescripcion As String) As Boolean

        Dim Objeto As New BE.Transaccion.BETransaccion
        Dim Servicio As New BL.Transaccion.BLTransaccion

        Objeto.nOpeCodigo = nOpeCodigo
        Objeto.cPerCodigo = cPerCodigo
        Objeto.cTraHost = "Win :" & My.Computer.Name
        Objeto.cTraDescripcion = cTraDescripcion

        If Servicio.Ins_Transaccion(Objeto) Then Return True Else Return False
    ...
    Function BuscarCodigoDGW(ByVal TextoABuscar As String, ByVal Columna As String, ByRef grid As DataGridView) As Boolean
    ...
    Public Sub BuscarCodigoDGW(ByRef DataGridView As DataGridView, ByVal Columna As String, ByVal TextoABuscar As String)

        Dim dt As DataTable = DirectCast(DataGridView.DataSource, DataTable)

        ' Creamos un objeto DataView al que le 
        ' pasaremos el objeto DataTable 
        ' 
        Dim view As DataView = New DataView(dt)

        ' Ordenamos la vista por el campo IdCliente, que es el campo que 
        ' vamos a utilizar para buscar el dato especificado. 
        ' 
        view.Sort = Columna '"IdCliente"

        ' Obtenemos la fila donde se encuentra el cliente buscado. 
        ' 
        Dim fila As Int32 = View.Find(TextoABuscar)

        If fila <> -1 Then
            ' Se ha encontrado el registro; establecemos la fila
            ' actual.
            '
            DataGridView.CurrentCell = DataGridView.Item(0, fila)
    ...
    ' función que retorna el total   
    Public Function Suma_Columna_SubTotal( _
        ByVal cCantidad As String, _
        ByVal cPrecio As String, _
        ByVal Dgv As DataGridView) As Double
        ...
        Try
            For i As Integer = 0 To Dgv.RowCount - 1
                If CDbl(Dgv.Item(cCantidad.ToLower, i).Value) > 0.0 And CDbl(Dgv.Item(cPrecio.ToLower, i).Value) > 0.0 Then
                    total = total + (CDbl(Dgv.Item(cCantidad.ToLower, i).Value) * CDbl(Dgv.Item(cPrecio.ToLower, i).Value))
                End If
            Next
            'CDbl
        Catch ex As Exception
    ...
    Public Sub ExportarDatosExcel(ByVal DataGridView1 As DataGridView, ByVal titulo As String, Optional ByVal rangofechas As String = "")
        ...
        Try
            ...
            With objHojaExcel
                ...
                'Encabezado   
                .Range("A1:L1").Merge()
                .Range("A1:L1").Value = "MÉDICA (Laboratorio Clínico Especializado)"
                ...
                Dim primerNumero As Short ' = 3

                If Len(rangofechas) = 0 Then
                    primerNumero = 3
                Else
                    .Range("A3:L3").Merge()
                    .Range("A3:L3").Value = rangofechas
                    .Range("A3:L3").Font.Bold = True
                    .Range("A3:L3").Font.Size = 11
                    primerNumero = 4
                End If
                ...
                For Each c As DataGridViewColumn In DataGridView1.Columns
                    If c.Visible Then
                        ...
                        ElseIf DataGridView1.Columns.Item(c.Index).DefaultCellStyle.Format = "d" Then
                            objCelda.EntireColumn.NumberFormat = "yyyy/MM/dd"
                        End If
                    End If
                Next
                ...
                For Each reg As DataGridViewRow In DataGridView1.Rows
                    ...
                    For Each c As DataGridViewColumn In DataGridView1.Columns
                        ...
                        If c.Visible Then
                            ...
                            If DataGridView1.Columns.Item(c.Index).DefaultCellStyle.Format = "dd/MM/yyyy" Then
                                .Cells(i, strColumna) = CDate(reg.Cells(c.Index).Value)
                            Else
                                .Cells(i, strColumna) = IIf(IsDBNull(reg.ToString), "", reg.Cells(c.Index).Value.ToString)
                            End If


                            '(eincio) .Cells(i, strColumna) = IIf(IsDBNull(reg.ToString), "", Chr(39) & Convert.ToString(reg.Cells(c.Index).Value))
    ...
    Public Function CalculaEdad(dtInicio As Date, dtFin As Date) As String
        ...
        MesesPasados = DateDiff("m", dtInicio, dtFin)

        Anios = Int(MesesPasados / 12)
        Meses = MesesPasados Mod 12

        If Microsoft.VisualBasic.Day(dtFin) >= Microsoft.VisualBasic.Day(dtInicio) Then
            dias = Microsoft.VisualBasic.Day(dtFin) - Microsoft.VisualBasic.Day(dtInicio)
        Else
            Select Case Month(dtFin)
                Case 1, 2, 4, 6, 8, 9, 11 : dias = 31 - Microsoft.VisualBasic.Day(dtInicio) + Microsoft.VisualBasic.Day(dtFin)
                Case 5, 7, 30, 12 : dias = 30 - Microsoft.VisualBasic.Day(dtInicio) + Microsoft.VisualBasic.Day(dtFin)
                Case 3 : dias = Bisiento(Year(dtFin)) - Microsoft.VisualBasic.Day(dtInicio) + Microsoft.VisualBasic.Day(dtFin)
            End Select
            Meses = Meses - 1
            If Meses = -1 Then
                Meses = 11
    ...
    Public Function CreateNodeXML(Elemento As String, Valor As String, _
                              WithCloseTag As Boolean, ParamArray Atributo() As Object) As String
        Dim strOut As String
        Dim NumAttrib As Long

        strOut = "<" & Elemento
        For NumAttrib = LBound(Atributo) To UBound(Atributo)
            If NumAttrib Mod 2 = 0 Then
                strOut = strOut & " " & Atributo(NumAttrib) & "="""
            Else
                strOut = strOut & ConvertXMLCharacter(CStr(Atributo(NumAttrib))) & """"
            End If
        Next
        If Right(strOut, 2) = "=""" And UBound(Atributo) >= 0 Then strOut = strOut & """"
        strOut = strOut & ">" & ConvertXMLCharacter(Valor)
        If WithCloseTag Then strOut = strOut & "</" & Elemento & ">"
        CreateNodeXML = strOut
    End Function

    Public Function ConvertXMLCharacter(Expresion As String) As String
        Dim StrIn As String
        StrIn = Replace(Expresion, "<", "&lt;")
        StrIn = Replace(StrIn, ">", "&gt;")
        StrIn = Replace(StrIn, "&", "&amp;")
        StrIn = Replace(StrIn, "'", "&apos;")
        StrIn = Replace(StrIn, """", "&quot;")
        StrIn = Replace(StrIn, "Á", "&#" & AscW("Á") & ";")
        StrIn = Replace(StrIn, "É", "&#" & AscW("É") & ";")
        StrIn = Replace(StrIn, "Í", "&#" & AscW("Í") & ";")
        StrIn = Replace(StrIn, "Ó", "&#" & AscW("Ó") & ";")
        StrIn = Replace(StrIn, "Ú", "&#" & AscW("Ú") & ";")
        StrIn = Replace(StrIn, "á", "&#" & AscW("á") & ";")
        StrIn = Replace(StrIn, "é", "&#" & AscW("é") & ";")
        StrIn = Replace(StrIn, "í", "&#" & AscW("í") & ";")
        StrIn = Replace(StrIn, "ó", "&#" & AscW("ó") & ";")
        StrIn = Replace(StrIn, "ú", "&#" & AscW("ú") & ";")
        StrIn = Replace(StrIn, "º", "&#" & AscW("º") & ";")
        StrIn = Replace(StrIn, "ñ", "&#" & AscW("ñ") & ";")
        StrIn = Replace(StrIn, "Ñ", "&#" & AscW("Ñ") & ";")

        ConvertXMLCharacter = StrIn
    End Function
End Module
